		...
		<v-layout row wrap>
			<v-flex xs12 v-if="loaded" ml-5 mr-5>
				<v-form @submit.prevent>
					<v-layout row wrap>
						<v-flex xs6>
							<v-text-field v-model="request.title" label="Title" autocomplete="off" :readonly="true" box></v-text-field>
						</v-flex>
						<v-flex xs6>
							<v-text-field
								v-model="startString"
						...
						<v-flex xs6>
							<v-text-field v-model="endString" label="End Date" autocomplete="off" :readonly="true" box></v-text-field>
						</v-flex>
						<v-flex xs6>
							<v-text-field v-model="request.halfDay" label="Half Day" :readonly="true" box></v-text-field>
						</v-flex>
						<v-flex xs6>
							<v-text-field v-model="this.convertLeaveType" label="Leave Type" :readonly="true" box></v-text-field>
							<v-text-field
						...
						<v-flex xs6>
							<v-text-field v-model="aggrStatus" label="Status" :readonly="true" box></v-text-field>
						</v-flex>
						<v-flex xs6>
							<v-text-field
						...
						<v-flex
							v-if="this.user.email == this.request.requestor && this.getStatus(this.request.aggregateStatus()) == 'Pending'"
							class="text-xs-center"
							mt-5
						>
							<v-btn color="reject" @click.stop="deleteRequest">Delete</v-btn>
						</v-flex>
						<v-flex v-if="canApproveReject" class="text-xs-center" mt-5>
							<v-btn color="approve" @click.stop="approve">Approve</v-btn>
							<v-btn color="reject" @click.stop="reject">Reject</v-btn>
						</v-flex>
					</v-layout>
				</v-form>
			</v-flex>
...
export default {
	...
	computed: {
		...
		canApproveReject() {
			...
			return (
				this.$store.state.loggedInUser.isApprover &&
	...
	methods: {
		...
		validateRejection() {
			...
			if (
				this.$store.state.loggedInUser.email === this.request.secondApprover
			) {
		...
		deleteRequest() {
			...
			this.$store
				.dispatch(action.DELETE_REQUEST, this.documentRef.id)
				.then(() => {
				...
				.then(() => {
					...
					if (["SICK", "COMP", "EXAM", "MAT", "PAT", "MAR"].includes(item.leaveType)) {
						...
						this.$store
							.dispatch(action.DELETE_SL, aggrString)
							.then(res => {
		...
		approveAction() {
			if (this.validateDate() == false) {
				...
				this.$store.commit(
					mutant.SET_ERROR,
					"the request seems exceeded the quota"
				);
			} else {
				...
				if (
					this.$store.state.loggedInUser.email === this.request.firstApprover &&
		...
		reject() {
			if (!this.validateRejection()) {
				this.$store.commit(
					mutant.SET_ERROR,
					"You must include a comment when rejecting"
				);
				return;
			}
					...
					events.forEach(entry => {
						...
						if (entry.halfDay != "Full") {
							...
							if (entry.leaveType == "ANN") {
								...
								console.log(
									"no. of days public holidays excluded",
									publicHolidayExclusion
								);
								this.approvedCo +=
									s.businessDiff(e) + 1 - publicHolidayExclusion;
		...
		validateDate() {
			...
			if (this.request.halfDay != "Full") {
				...
				if (this.request.leaveType == "ANN") {
					...
					//deal with floats
					if (
						(a.businessDiff(b) + 1 - publicHolidayExclusion) * 10 >
						Math.round(
							(this.userDetails.daysAnnualLeave - this.approvedAnn) * 10
						)
					) {
						return false;
					}
					...
					console.log(
						"remaining should be = ",
						this.userDetails.daysCarryOver,
						"-",
						this.approvedCo,
						"+",
						publicHolidayExclusion
					...
					if (
						(a.businessDiff(b) + 1 - publicHolidayExclusion) * 10 >
						Math.round((this.userDetails.daysCarryOver - this.approvedCo) * 10)
					) {
						return false;
					}
				}
		...
		fetchUser() {
			this.$store
				.dispatch(action.GET_USER, { email: this.request.requestor })
				.then(user => {
